...
if Modules == nil then
	...
	function StdModule.travel(cid, message, keywords, parameters, node)
		...
		if player:isPremium() or not parameters.premium then
			...
			elseif not player:removeTotalMoney(parameters.cost) then
				...
				npcHandler:say(parameters.msg or "Teleportando...", cid)
	...
	function ShopModule:callbackOnSell(cid, itemid, subType, amount, ignoreEquipped, _)
		...
		local player = Player(cid)
		local totalMoney = amount * shopItem.sell

		local parseInfo = {
			[TAG_PLAYERNAME] = player:getName(),
			[TAG_ITEMCOUNT] = amount,
			[TAG_TOTALCOST] = totalMoney,
			[TAG_ITEMNAME] = shopItem.name
		}
		...
		if player:removeItem(itemid, amount, subType, ignoreEquipped) then
			...
			local coinValue = 10000
			local coinCount = math.floor(totalMoney / coinValue)

			if coinCount > 0 then
				player:addItem(crystalId, coinCount)
			end
