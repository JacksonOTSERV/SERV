...
if NpcHandler == nil then
	...
	function NpcHandler:parseMessage(msg, parseInfo)
		if type(msg) == "table" then
			...
			for k, v in pairs(msg) do
				newMsg[k] = self:parseMessage(v, parseInfo)
			end
			return newMsg
		end
	...
	function NpcHandler:say(message, focus, publicize, shallDelay, delay)
		if type(message) == "table" and (message.PT or message.EN) then
			local lang = 0
			local player = Player(focus)
			if player then
				lang = math.max(0, player:getStorageValue(45001))
			end
			
			if lang == 1 and message.EN then
				message = message.EN
			elseif message.PT then
				message = message.PT
