...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if targetType.shrine then
		...
		player:getPosition():sendMagicEffect(items.valuables.effect, player)
		item:remove(1)
	else
		...
		player:getPosition():sendMagicEffect(targetItem.effects and (isInSlot and targetItem.effects.failure or targetItem.effects.success, player) or items.equipment.effect)
	end
	return true
end
