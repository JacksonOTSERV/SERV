...
combat:setParameter(COMBAT_PARAM_DISTANCEEFFECT, 49)
...
function onCastSpell(creature, variant)
    ...
    for k = 1, 7 do
        addEvent(function()
            ...
            if currentCreature and targetCreature and not targetCreature:isInGhostMode() then
				...
				local effectPos = {x = playerPos.x + 2, y = playerPos.y + 2, z = playerPos.z}
				doSendMagicEffect(effectPos, 311, currentCreature)
    ...
    if sendSpellbarCooldownAuto then
        sendSpellbarCooldownAuto(creature, "Overheating blast")
    end
