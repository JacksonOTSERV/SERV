...
combat:setParameter(COMBAT_PARAM_DISTANCEEFFECT, 68)
    ...
    for k = 1, 10 do
        addEvent(function()
            ...
            if currentCreature and targetCreature and not targetCreature:isInGhostMode() then
                ...
                local effectPos = Position(targetCreature:getPosition().x + 1, targetCreature:getPosition().y + 1, targetCreature:getPosition().z)
                effectPos:sendMagicEffect(59)
            end
        end, 1 + ((k - 1) * 225))
    end
