...
function onGetFormulaValues(creature, level, maglevel)
    local min = -(DAMAGE_FACTOR_LEVEL1 * level + DAMAGE_FACTOR_SKILL1 * maglevel) * 0.78
    local max = -(DAMAGE_FACTOR_LEVEL1 * level + DAMAGE_FACTOR_SKILL1 * maglevel) * 0.82
    return min, max 
end
...
function onCastSpell(creature, variant)
    ...
    if sendSpellbarCooldownAuto then
        sendSpellbarCooldownAuto(creature, "Generic ki blast")
    end
