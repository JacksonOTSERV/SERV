...
function onCastSpell(creature, cid, variant)
    ...
    local effectPos = {x = targetPos.x + 3, y = targetPos.y + 3, z = targetPos.z}
    local positions = {
    ...
    for _, fromPos in ipairs(positions) do
        doSendDistanceShoot(fromPos, target:getPosition(), 98)
    end
    doSendMagicEffect(effectPos, 301)
    ...
    if sendSpellbarCooldownAuto then
        sendSpellbarCooldownAuto(creature, "Tengai saikosai")
    end
    return true
end
