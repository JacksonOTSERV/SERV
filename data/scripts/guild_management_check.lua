...
local function handleGuildAction(player, action, data)
    ...
    elseif action == "levelUp" then
        ...
        print("DEBUG: Cost: " .. cost .. " | Balance: " .. guild:getBankBalance())
...
-- Startup Persistence Check (disabled)
--[[
addEvent(function()
    print(">>> PERSISTENCE CHECK <<<")
    local check = db.storeQuery("SELECT `id`, `name`, `level` FROM `guilds`")
    if check then
        repeat
            local gid = result.getNumber(check, "id")
            local name = result.getString(check, "name")
            local level = result.getNumber(check, "level")
            print("GUILD [" .. gid .. "] " .. name .. " - LEVEL: " .. level)
        until not result.next(check)
        result.free(check)
    else
        print("NO GUILDS FOUND IN DB")
    end
    print(">>> END CHECK <<<")
end, 1000)
]]
