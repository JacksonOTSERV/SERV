...
function doSendMagicEffect(pos, magicEffect, creature)
    local caster = creature or _spellCaster
    return Position(pos):sendMagicEffect(magicEffect, caster)
end
function doSendDistanceShoot(fromPos, toPos, distanceEffect, creature)
    local caster = creature or _spellCaster
    return Position(fromPos):sendDistanceEffect(toPos, distanceEffect, caster)
end
...
function Guild.getMembers(self)
	local members = {}
	local resultId = db.storeQuery("SELECT `name` FROM `players` WHERE `id` IN (SELECT `player_id` FROM `guild_membership` WHERE `guild_id` = " .. self:getId() .. ")")
	if resultId ~= false then
		repeat
			local name = result.getString(resultId, "name")
			local player = Player(name)
...
function Guild.getMembersOnline(self)
	local members = {}
	local resultId = db.storeQuery("SELECT `name` FROM `players` WHERE `id` IN (SELECT `player_id` FROM `guild_membership` WHERE `guild_id` = " .. self:getId() .. ")")
	if resultId ~= false then
		repeat
...
function Guild.getBankBalance(self)
	local resultId = db.storeQuery("SELECT `balance` FROM `guilds` WHERE `id` = " .. self:getId())
	if resultId ~= false then
...
function Guild.setBankBalance(self, balance)
	db.query("UPDATE `guilds` SET `balance` = " .. tonumber(balance) .. " WHERE `id` = " .. self:getId())
end
