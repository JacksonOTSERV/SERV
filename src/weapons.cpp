...
bool Weapon::useFist(Player* player, Creature* target)
{
    ...
    double criticalHitChance = player->getCriticalHitChance() + g_config.getNumber(ConfigManager::CRITICAL_HIT_CHANCE);
    
    int32_t randomValue = rand() % 100 + 1;
    ...
    if (targetPlayer) {
        int32_t shieldingSkill = targetPlayer->getSkillLevel(SKILL_SHIELD);
        dodgeChance = shieldingSkill / 5.0;
        dodgeChance = std::min(dodgeChance, 100.0);
    }

    randomValue = rand() % 100 + 1;

    if (dodgeChance >= randomValue) {
        maxDamage = std::floor(maxDamage * 0.25);
        target->sendDodge();
    }
    ...
    combatDamage.primary.type = params.combatType;
    combatDamage.primary.value = damage;

    Combat::doTargetCombat(player, target, combatDamage, params);

    if (!player->hasFlag(PlayerFlag_NotGainSkill) && player->getAddAttackSkill()) {
        if (player->getBaseSkill(SKILL_FIST) < 130) {
            player->addSkillAdvance(SKILL_FIST, 1);
        }
        if (player->getBaseSkill(SKILL_FISHING) < 90) {
            player->addSkillAdvance(SKILL_FISHING, 1);
        }
    }
	...
	if (!player->hasFlag(PlayerFlag_NotGainSkill)) {
		if (item->getID() == 13603) {
			...
			if (getSkillType(player, item, skillType, skillPoint)) {
				if (player->getBaseSkill(SKILL_CLUB) < 90)
					player->addSkillAdvance(SKILL_CLUB, skillPoint);
				if (player->getBaseSkill(SKILL_SWORD) < 90)
			...
			uint32_t skillPoint;
			if (getSkillType(player, item, skillType, skillPoint)) {
				if (player->getBaseSkill(skillType) < 90) {
...
bool WeaponMelee::useWeapon(Player* player, Item* item, Creature* target) const
{
    ...
    if (player->getCurrentOutfit().lookAura == 1264) {
        damageModifier = playerWeaponCheck(player, target, 6);
        
        if (damageModifier > 0) {
            g_game.addDistanceEffect(player->getPosition(), target->getPosition(), 17, player->getInstanceID());
        }
    } else {
...
int32_t WeaponMelee::getElementDamage(const Player* player, const Creature* target, const Item* item) const
{
    ...
    bool isCritical = criticalHitChance >= randomValue;
    if (isCritical) {
        maxValue = static_cast<int32_t>(maxValue * g_config.getDouble(ConfigManager::CRITICAL_HIT_MUL));
        target->sendCritical();
    }
	
	Player* targetPlayer = dynamic_cast<Player*>(const_cast<Creature*>(target));
    double dodgeChance = 0.0;
    ...
    if (dodgeChance >= randomValue) {
        maxValue = std::floor(maxValue * 0.25);
        target->sendDodge();
    }
...
{
	struct WeaponCritEffect {
		uint8_t effectId;
		int offsetX;
		int offsetY;
	};

	static const std::unordered_map<uint16_t, WeaponCritEffect> critEffects = {
		{13570, {62, 1, 0}},
		{13571, {52, 1, 0}},
		{12714, {47, 1, 1}},
		{13466, {9, 0, 0}},
	...
	static std::unordered_map<uint32_t, std::pair<uint32_t, uint8_t>> hitTracker;

	if (item->getID() == 13603 && target->getPlayer()) {
		uint32_t attackerId = player->getID();
		uint32_t targetId = target->getID();

		auto& entry = hitTracker[attackerId];

		if (entry.first == targetId) {
			entry.second++;
		} else {
			entry.first = targetId;
			entry.second = 1;
		}

		if (entry.second >= 8) {
			Condition* stun = Condition::createCondition(CONDITIONID_COMBAT, CONDITION_STUN, 1500, 0);
			if (stun) {
				Creature* tCreature = const_cast<Creature*>(target);
				tCreature->addCondition(stun);

				g_game.addAnimatedText("Stunned!", target->getPosition(), TEXTCOLOR_PURPLE, target->getInstanceID());
			}
			entry.second = 0;
		}
	}

	auto calculateHit = [&](bool& isCriticalHit, bool& isDodged) -> int32_t {
		int32_t attackSkill;

		if (item->getID() == 13603) {
			int32_t clubSkill = player->getSkillLevel(SKILL_CLUB);
			int32_t swordSkill = player->getSkillLevel(SKILL_SWORD);

			if (clubSkill > swordSkill) {
				attackSkill = clubSkill;
			} else if (swordSkill > clubSkill) {
				attackSkill = swordSkill;
			} else {
				attackSkill = (rand() % 2 == 0) ? clubSkill : swordSkill;
			}
		} else {
			attackSkill = player->getWeaponSkill(item);
		}

		int32_t attackValue = std::max<int32_t>(0, item->getAttack() * 20);
		float attackFactor = player->getAttackFactor();
		...
		int32_t fistSkill = player->getSkillLevel(SKILL_FIST);
		float fistMultiplier = 1.0f + (static_cast<float>(fistSkill) / 100.0f);
		baseDamage = static_cast<int32_t>(baseDamage * fistMultiplier);

		double criticalHitChance = player->getCriticalHitChance() + g_config.getNumber(ConfigManager::CRITICAL_HIT_CHANCE);
		int32_t roll = rand() % 100 + 1;
		isCriticalHit = (criticalHitChance >= roll);
		if (isCriticalHit) {
			baseDamage = static_cast<int32_t>(baseDamage * g_config.getDouble(ConfigManager::CRITICAL_HIT_MUL));
		}
		...
		roll = rand() % 100 + 1;
		isDodged = (dodgeChance >= roll);
		if (isDodged) {
			baseDamage = std::floor(baseDamage * 0.25);
		}

		return maxDamage ? baseDamage : normal_random(0, baseDamage);
	};

	bool crit1 = false, dodge1 = false;
	bool crit2 = false, dodge2 = false;

	int32_t damage1 = calculateHit(crit1, dodge1);
	int32_t damage2 = calculateHit(crit2, dodge2);

	if (crit1 || crit2) {
		auto it = critEffects.find(item->getID());
		if (it != critEffects.end()) {
			const WeaponCritEffect& effectData = it->second;

			Position effectPos = target->getPosition();
			effectPos.x += effectData.offsetX;
			effectPos.y += effectData.offsetY;

			g_game.addMagicEffect(effectPos, effectData.effectId, target->getInstanceID());
		}
	}

	if (crit1) target->sendCritical();
	if (crit2) target->sendCritical();

	if (dodge1) target->sendDodge();
	if (dodge2) target->sendDodge();
