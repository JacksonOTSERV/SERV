...
Player::~Player() {
  // Clean spectating refs to avoid dangling ptrs
  if (spectating) {
    spectating->removeSpectator(this);
  }
  for (Player* spec : spectators) {
    spec->setSpectating(nullptr);
  }
...
void Player::setSkillLevel(uint64_t skillType, uint64_t newSkillLevel) {
  ...
  skills[skillType].percent = Player::getPercentLevel(
      skills[skillType].tries,
      vocation->getReqSkillTries(skillType, skills[skillType].level));
...
std::string Player::getDescription(int32_t lookDistance) const {
  ...
  s << guildRank->name << " da guild " << guild->getName();
  if (!guildNick.empty()) {
    s << " (" << guildNick << ')';
  }
...
int32_t Player::getArmor() const {
  ...
  static const slots_t armorSlots[] = {CONST_SLOT_HEAD,   CONST_SLOT_NECKLACE,
                                       ...
                                       CONST_SLOT_AMULET, CONST_SLOT_AMULET2,
                                       CONST_SLOT_ANEL,   CONST_SLOT_ANEL2};
...
void Player::getShieldAndWeapon(const Item *&_shield,
                                const Item *&_weapon) const {
  _shield = NULL;
  Item *item = NULL;
...
void Player::addSkillAdvance(skills_t skill, uint64_t count) {
  ...
  while ((skills[skill].tries + count) >= nextReqTries) {
    ...
    ss << "Você avançou para " << getSkillName(skill) << " level "
       << skills[skill].level << '.';
...
uint16_t Player::getLookCorpse() const {
  const Vocation *vocation = getVocation();
  if (vocation) {
    uint16_t corpse = vocation->getCorpse();
    if (corpse > 0) {
      return corpse;
    }
  }
...
bool Player::addKoteElemental(uint32_t koteIdBg) {
  std::map<uint32_t, uint32_t>::iterator it = koteBgs.find(koteIdBg);
  if (it != koteBgs.end())
    return false;

  koteBgs[koteIdBg] = koteIdBg;
  return true;
}

bool Player::removeKoteElemental(uint32_t koteIdBg) {
  std::map<uint32_t, uint32_t>::iterator it = koteBgs.find(koteIdBg);
  if (it == koteBgs.end())
    return false;
...
void Player::addStorageValue(const uint32_t key, const int32_t value,
  ...
  if (IS_IN_KEYRANGE(key, RESERVED_RANGE)) {
    if (IS_IN_KEYRANGE(key, OUTFITS_RANGE)) {
      ...
      return;
    } else if (IS_IN_KEYRANGE(key, HEALTHBG_RANGE)) {
      uint32_t healthBgId = value >> 16;
      addHealthBackground(healthBgId);
      return;
    } else if (IS_IN_KEYRANGE(key, MANABG_RANGE)) {
      uint32_t manaBgId = value >> 16;
      addManaBackground(manaBgId);
      return;
    } else if (IS_IN_KEYRANGE(key, KOTEBG_RANGE)) {
      uint32_t koteBgId = value >> 16;
      addKoteElemental(koteBgId);
      return;
    } else if (IS_IN_KEYRANGE(key, WINGS_RANGE) ||
...
void Player::onReceiveMail() const {
  if (isNearDepotBox()) {
    sendTextMessage(
        ...
        "Sua compra do market chegou no seu mailbox, em seu depot.");
  }
}
...
void Player::setWriteItem(Item *item, uint16_t maxWriteLen) {
  windowTextId++;
...
void Player::sendRemoveContainerItem(const Container *container,
  ...
  for (auto &it : openContainers) {
    ...
    client->sendRemoveContainerItem(it.first,
                                    std::max<uint16_t>(slot, firstIndex));
  }
}
...
void Player::onThink(uint32_t interval) {
  ...
  if (!getTile()->hasFlag(TILESTATE_NOLOGOUT) && !isAccessPlayer()) {
    ...
    if (idleTime > (kickAfterMinutes * 60000) + 60000) {
      ...
      std::ostringstream ss;
      ss << "N? houve varia?es no seu comportamento por " << kickAfterMinutes
         << " minutos. Voc? ser? desconectado em um minuto se n? houver "
            "altera?es em suas atitudes. Atente-se.";
...
void Player::addManaSpent(uint64_t amount) {
  ...
  const uint32_t maxMagicLevel = 150;
  uint64_t currReqMana = vocation->getReqMana(magLevel);
  uint64_t nextReqMana = vocation->getReqMana(magLevel + 1);
  if (currReqMana >= nextReqMana || magLevel >= maxMagicLevel) {
    return;
  }
  ...
  while ((manaSpent + amount) >= nextReqMana) {
    ...
    std::ostringstream ss;
    ss << "Voc? avan?u para o Ki Level " << magLevel << '.';
    sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
...
void Player::addExperience(Creature *source, uint64_t exp,
  ...
  if (sendText) {
    std::string expString =
        std::to_string(exp) +
        (exp != 1 ? " pontos de experi?cia." : " ponto de experi?cia.");
    ...
    SpectatorVec instanceSpectators = InstanceUtils::filterByInstance(spectators, getInstanceID());
    g_game.addAnimatedText(instanceSpectators, strExp.str(), position, TEXTCOLOR_WHITE);

    if (!instanceSpectators.empty()) {
      message.type = MESSAGE_STATUS_DEFAULT;
      message.text = getName() + " ganhou " + expString;
      for (Creature *spectator : instanceSpectators) {
  ...
  if (prevLevel != level) {
    ...
    std::ostringstream ss;
    ss << "Voc? avan?u do Level " << prevLevel << " para o Level " << level
       << '.';
    sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
  }
...
void Player::removeExperience(uint64_t exp, bool sendText /* = false*/) {
  ...
  if (sendText) {
    ...
    spectators.erase(this);
    SpectatorVec instanceSpectators = InstanceUtils::filterByInstance(spectators, getInstanceID());
    g_game.addAnimatedText(instanceSpectators, strExp.str(), position, TEXTCOLOR_RED);
  ...
  if (oldLevel != level) {
    ...
    std::ostringstream ss;
    ss << "Voc? foi rebaixado do Level " << oldLevel << " para o Level "
       << level << '.';
    sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
    sendReLoginWindow();
  }
...
void Player::onBlockHit() {
  const uint32_t maxShieldSkillLevel = 90;
  uint32_t currentShieldSkillLevel = getSkillLevel(SKILL_SHIELD);

  if (currentShieldSkillLevel < maxShieldSkillLevel) {
...
BlockType_t Player::blockHit(Creature *attacker, CombatType_t combatType,
  ...
  // Start Powerup Stats (Defensive)
  std::cout << "[DEBUG] Player::blockHit: Incoming Damage: " << damage
            << std::endl;

  // Resilience (50004)
  int32_t resilience = 0;
  if (getStorageValue(50004, resilience)) {
    std::cout << "[DEBUG] Resilience found: " << resilience << std::endl;
  } else {
    std::cout << "[DEBUG] Resilience NOT found" << std::endl;
  }
  if (getStorageValue(50004, resilience) && resilience > 0) {
    damage -= std::round(damage * (resilience / 100.0f));
    if (damage < 0)
      damage = 0;
  }

  // Dodge (50010)
  int32_t dodgeChance = 0;
  if (getStorageValue(50010, dodgeChance)) {
    std::cout << "[DEBUG] Dodge found: " << dodgeChance << std::endl;
  }

  if (dodgeChance > 0) {
    // Using standard random number generation consistent with codebase
    // Assuming normal_random is available via tool.h or otpch.h which are
    // likely included. If normal_random isn't available, we use std::rand() or
    // similar. But let's stick to the codebase style. Combat uses
    // normal_random(min, max). Actually normal_random returns a value in range
    // [min, max].
    if (normal_random(1, 3) <= 1) { // Dodge every 1-3 hits
      damage = 0;
      sendTextMessage(MESSAGE_STATUS_SMALL, "DODGE!");
      if (attacker) {
...
void Player::death(Creature *lastHitCreature) {
  (void)lastHitCreature;

  uint32_t oldLevel = level;

  loginPosition = town->getTemplePosition();
  bool skillPerder = true;

  uint32_t townId = 0;
  Database &db = Database::getInstance();
  std::ostringstream query;
  query << "SELECT town_id FROM players WHERE id = " << this->getGUID();
  DBResult_ptr result = db.storeQuery(query.str());
  if (result) {
    townId = result->getNumber<uint32_t>("town_id");
  } else {
    std::cout << "[Error - Player::death] Nao foi possivel encontrar o town_id "
                 "para o jogador "
              << name << std::endl;
    return;
  }

  uint32_t additionalLevelLoss = 0;
  if (townId == 6) {
    additionalLevelLoss = 2;
  }
  ...
  if (skillPerder) {
    ...
    uint32_t levelLoss = (blessings > 0) ? 2 : 4;
    levelLoss += additionalLevelLoss;

    if (getSkull() == SKULL_RED)
      levelLoss += 2;

    level = std::max<uint32_t>(1, level - levelLoss);

    uint32_t lostLevels = oldLevel - level;
    healthMax = std::max<int32_t>(
        healthMax - lostLevels * vocation->getHPGain(), vocation->getHPGain());
    manaMax = std::max<int32_t>(manaMax - lostLevels * vocation->getManaGain(),
  ...
  if (corpse && corpse->getContainer()) {
    ...
    if (lastHitCreature) {
      ss << "Voc? v? " << getNameDescription() << ". "
         << (getSex() == PLAYERSEX_FEMALE ? "Ele foi morto" : "Ele foi morto")
         << " por " << lastHitCreature->getNameDescription() << '.';
    } else {
      ss << "Voc? " << getNameDescription() << '.';
    }
...
void Player::addCombatExhaust(uint32_t ticks) {
  Condition *condition = Condition::createCondition(
      CONDITIONID_DEFAULT, CONDITION_EXHAUST_COMBAT, ticks, 0);
  addCondition(condition);
}

void Player::addHealExhaust(uint32_t ticks) {
  Condition *condition = Condition::createCondition(
      CONDITIONID_DEFAULT, CONDITION_EXHAUST_HEAL, ticks, 0);
  addCondition(condition);
}
...
void Player::addInFightTicks(bool pzlock /*= false*/, uint64_t ticks) {
  ...
  Condition *condition = Condition::createCondition(
      CONDITIONID_DEFAULT, CONDITION_INFIGHT, ticks, 0);
  addCondition(condition);
  sendIcons();
}
...
bool Player::addVIP(uint32_t vipGuid, const std::string &vipName,
  ...
  if (VIPList.size() >= getMaxVIPEntries() ||
    ...
    sendTextMessage(MESSAGE_STATUS_SMALL,
                    "Voc? n? pode adicionar mais amigos.");
    return false;
  }
...
ReturnValue Player::queryAdd(int32_t index, const Thing &thing, uint32_t count,
  ...
  if (slotPosition & SLOTP_ANEL) {
    ret = RETURNVALUE_ONLYCRYSTALRINGHERE;
  } else if (slotPosition & SLOTP_ANEL2) {
    ret = RETURNVALUE_ONLYMOONRINGHERE;
  } else if (slotPosition & SLOTP_AMULET) {
    ret = RETURNVALUE_ONLYPHOENIXAMULETHERE;
  } else if (slotPosition & SLOTP_AMULET2) {
    ret = RETURNVALUE_ONLYPOWERAMULETHERE;
  } else if ((slotPosition & SLOTP_HEAD) || (slotPosition & SLOTP_NECKLACE) ||
  ...
  case CONST_SLOT_AMULET: {
    if (slotPosition & SLOTP_AMULET) {
      ret = RETURNVALUE_NOERROR;
    } else {
      ret = RETURNVALUE_ONLYPHOENIXAMULETHERE;
    }
  ...
  case CONST_SLOT_AMULET2: {
    if (slotPosition & SLOTP_AMULET2) {
      ret = RETURNVALUE_NOERROR;
    } else {
      ret = RETURNVALUE_ONLYPOWERAMULETHERE;
    }
  ...
  case CONST_SLOT_ANEL2: {
    if (slotPosition & SLOTP_ANEL2) {
      ret = RETURNVALUE_NOERROR;
    } else {
      ret = RETURNVALUE_ONLYMOONRINGHERE;
    }
  ...
  // need an exchange with source? (destination item is swapped with currently
  // moved item)
  ...
  if (index == INDEX_WHEREEVER) {
          ...
          for (ContainerIterator it = subContainer->iterator(); it.hasNext();
            ...
            if (Container *tmpContainer = (*it)->getContainer()) {
              ...
              n += queryCount;
                   ...
                   inventoryItem->getItemCount() < MAX_ITEM_COUNT) {
    ...
    if (destItem) {
      if (destItem->isStackable() && item->equals(destItem) &&
          destItem->getItemCount() < MAX_ITEM_COUNT) {
...
Cylinder *Player::queryDestination(int32_t &index, const Thing &thing,
  ...
  if (index == 0 /*drop to capacity window*/ || index == INDEX_WHEREEVER) {
    *destItem = nullptr;
    ...
    for (uint32_t slotIndex = CONST_SLOT_FIRST; slotIndex <= CONST_SLOT_LAST;
         ++slotIndex) {
      Item *inventoryItem = inventory[slotIndex];
      if (inventoryItem) {
    ...
    while (i < containers.size()) {
      ...
      if (!autoStack || !isStackable) {
        ...
        uint32_t n = tmpContainer->capacity() -
                     std::min(tmpContainer->capacity(),
                              static_cast<uint32_t>(tmpContainer->size()));
        while (n) {
...
void Player::removeThing(Thing *thing, uint32_t count) {
  ...
  if (item->isStackable()) {
    if (count == item->getItemCount()) {
      ...
      uint16_t newCount = static_cast<uint16_t>(
          std::max<int32_t>(0, item->getItemCount() - count)); // stack >255: count é uint16
...
void Player::postAddNotification(Thing *thing, const Cylinder *oldParent,
  ...
  if (link == LINK_OWNER) {
    ...
    g_events->eventPlayerOnInventoryUpdate(this, thing->getItem(),
                                           static_cast<slots_t>(index), true);
  }
...
bool Player::updateSaleShopList(const Item *item) {
  ...
  if (itemId != ITEM_GOLD_COIN && itemId != ITEM_PLATINUM_COIN &&
      itemId != ITEM_CRYSTAL_COIN && itemId != ITEM_GOLD_INGOT &&
      itemId != ITEM_DIAMOND_COIN && itemId != ITEM_ONLINE_TOKEN &&
      itemId != ITEM_EVENT_TOKEN) {
...
void Player::onAddCombatCondition(ConditionType_t type) {
  ...
  case CONDITION_DRUNK:
    sendTextMessage(MESSAGE_STATUS_DEFAULT, "Voc? est? desnorteado.");
    break;

  case CONDITION_CURSED:
    sendTextMessage(MESSAGE_STATUS_DEFAULT, "Voc? est? amaldi?ado.");
    break;
  ...
  case CONDITION_STUN:
    sendTextMessage(MESSAGE_STATUS_DEFAULT, "Voc? est? atordoado.");
    break;

  case CONDITION_MOVE:
    sendTextMessage(MESSAGE_STATUS_DEFAULT,
                    "Voc? est? impossibilitado de andar.");
    break;
...
void Player::addUnjustifiedDead(const Player *attacked) {
  ...
  sendTextMessage(MESSAGE_EVENT_ADVANCE, "Aten?o! O assassinato de " +
                                             attacked->getName() +
                                             " n? foi justificado.");
  ...
  Database &db = Database::getInstance();
  std::ostringstream query;
  query << "INSERT INTO player_frags (killer_id, target_id, frag_time) VALUES ("
        << getGUID() << ", " << attacked->getGUID() << ", " << now << ")";
  db.executeQuery(query.str());

  skullTicks += g_config.getNumber(ConfigManager::FRAG_TIME);

  updateSkullBasedOnFrags();
}

void Player::updateSkullBasedOnFrags() {
  int64_t now = OTSYS_TIME();
  int64_t fragDuration = 43200000;

  Database &db = Database::getInstance();
  std::ostringstream query;
  query << "SELECT COUNT(*) AS total FROM player_frags WHERE killer_id = "
        << getGUID() << " AND (frag_time + " << fragDuration << ") > " << now;
  ...
  uint32_t totalFrags = result->getNumber<uint32_t>("total");

  uint32_t killsToBlack = 100000000;
  uint32_t killsToRed = 15;

  Skulls_t currentSkull = getSkull();
  Skulls_t newSkull = SKULL_NONE;

  if (killsToBlack != 0 && totalFrags >= killsToBlack) {
    newSkull = SKULL_BLACK;
  } else if (killsToRed != 0 && totalFrags >= killsToRed) {
    newSkull = SKULL_RED;
  }

  if (currentSkull == SKULL_WHITE && newSkull == SKULL_NONE) {
    newSkull = SKULL_WHITE;
  }

  if (currentSkull != newSkull) {
    setSkull(newSkull);
  }
}
...
uint32_t Player::getAttackSpeed() const {
  uint32_t baseAttackSpeed = 2000;
  uint32_t minAllowedSpeed = 200;

  uint32_t fistSkill = getSkillLevel(SKILL_FIST);

  float progress = static_cast<float>(fistSkill) / 130.0f;
  progress = std::pow(progress, 0.75f);

  uint32_t speed = static_cast<uint32_t>(baseAttackSpeed -
                                         progress * (baseAttackSpeed - 500));

  return std::max(speed, minAllowedSpeed);
}
...
void Player::setPremiumDays(int32_t v) { premiumDays = v; }
...
int32_t Player::getCriticalHitChance() const {
  ...
  crit += std::max<int32_t>(0, getSkillLevel(SKILL_FISHING) / 2.85);

  return crit;
}

std::vector<std::pair<Creature*, int32_t>> Player::captureVisibleCreatures()
{
  // Kept for API compatibility — no longer used by applyInstanceCreatureSwitch
  return {};
}

void Player::applyInstanceCreatureSwitch(const std::vector<std::pair<Creature*, int32_t>>&)
{
  if (!client) {
    return;
  }
  // Full map refresh — replaces all tile data in viewport with new-instance-filtered content.
  // This is identical to what happens on teleport and is the only reliable way to sync
  // client state after an instance change (surgical per-creature remove/add has stackpos
  // ordering issues when multiple instances share the same tiles).
