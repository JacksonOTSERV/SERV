...
static constexpr int32_t PLAYER_MIN_SPEED = 250;
...
public:
  ...
  void autoLootOnKill(Creature *target);
  bool canCarryItem(const Item *item) const;
  uint32_t getFreeSlotsFromContainer(const Container *container) const;
  bool moveItemToPlayer(Item *item);
  ...
  void setDirectionalSpellAngle(float angle) { directionalSpellAngle = angle; }
  float getDirectionalSpellAngle() const { return directionalSpellAngle; }
  ...
  uint16_t getSoul() const { return soul; }
  ...
  uint16_t getSkillLevel(uint8_t skill) const {
    return static_cast<uint16_t>(
        std::max<int32_t>(0, skills[skill].level + varSkills[skill]));
  }
  ...
  void addInFightTicks(bool pzlock = false, uint64_t ticks = 0);
  ...
  // tile
  // send methods
  void sendAddTileItem(const Tile *tile, const Position &pos,
                       const Item *item) {
    if (spectating) return;
    if (client) {
  ...
  void sendChannelMessage(const std::string &author, const std::string &text,
    ...
    if (spectating) return; // recebe via forward do host
    if (client) {
  ...
  void sendAnimatedText(const std::string &message, const Position &pos,
                        TextColor_t color) {
    if (spectating) return;
    if (client) {
  ...
  void refreshWorldView() {
    if (client) {
      // Send a dummy MapDescription on a different Z index to FORCE OTClient to wipe
      // all existing ghost creatures from the screen. Then immediately send the real one.
      Position fakePos = getPosition();
      fakePos.z = (fakePos.z < 15) ? fakePos.z + 1 : fakePos.z - 1;
      client->sendMapDescription(fakePos);
      client->sendMapDescription(getPosition());
    }
  }
  std::vector<std::pair<Creature*, int32_t>> captureVisibleCreatures(); // deprecated noop
  // Call AFTER setInstanceID — sends full map refresh to sync client to new instance
  void applyInstanceCreatureSwitch(const std::vector<std::pair<Creature*, int32_t>>& oldVisible);
  ...
  bool stopLiveCast() { return client && client->stopLiveCast(); }
  bool isLiveCaster() const { return client && client->isLiveCaster(); }

  // PokeMonster-style in-game spectating
  void setSpectating(Player* target);
  bool isSpectator() const { return spectating != nullptr; }
  ...
  bool addKoteElemental(uint32_t koteIdBg);
  bool removeKoteElemental(uint32_t koteIdBg);
  ...
  void setFPS(uint16_t value) { fps = value; }
  void setLocalPing(uint16_t value) { localPing = value; }
  uint16_t getFPS() const { return fps; }
  uint16_t getLocalPing() const { return localPing; }
  ...
  std::map<uint32_t, uint32_t> koteBgs;
...
private:
  ...
  // PokeMonster spectating
  Player* spectating = nullptr;
  std::vector<Player*> spectators;

  std::string m_hwid; // anti-multicliente
  ...
  // Directional spell targeting
  float directionalSpellAngle = 0.0f;
  ...
  virtual int32_t getStepSpeed() const {
    ...
    if (this->getCurrentOutfit().lookType == 729) {
      const int32_t SPECIAL_MAX_SPEED = 5000;
      if (speed > SPECIAL_MAX_SPEED)
        return SPECIAL_MAX_SPEED;
  ...
  void updateBaseSpeed() {
    // Fixed 1000 base speed for all players.
    baseSpeed = 1000;
  }
