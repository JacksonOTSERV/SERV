...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_CRITICALHITCHANCE: {
			std::string criticalHitChance;
			if (!propStream.readString(criticalHitChance)) {
				return ATTR_READ_ERROR;
			}

			setStrAttr(ITEM_ATTRIBUTE_CRITICALHITCHANCE, criticalHitChance);
			break;
		}
...
bool Item::unserializeAttr2(PropStream& propStream)
{
	uint8_t attr_type;
	while (propStream.read<uint8_t>(attr_type) && attr_type != 0) {
		Attr_ReadValue ret = readAttr2(static_cast<AttrTypes_t>(attr_type), propStream);
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ITEM_ATTRIBUTE_CRITICALHITCHANCE)) {
		...
		propWriteStream.writeString(getStrAttr(ITEM_ATTRIBUTE_CRITICALHITCHANCE));
	}
...
{
	int64_t remainingDuration = g_game.getDecayTime(this);
	if (remainingDuration == -1) {
		remainingDuration = getInternalDuration();
	}
                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.isRune()) {
		if (it.runeLevel > 0 || it.runeMagLevel > 0) {
			if (RuneSpell* rune = g_spells->getRuneSpell(it.id)) {
				...
				s << ". " << (it.stackable && tmpSubType > 1 ? "Eles" : "Isto") << " s??de ser usado por ";
		...
		if (it.abilities) {
			...
			if (it.abilities->stats[STAT_MAGICPOINTS]) {
				...
				s << "ki level " << std::showpos << it.abilities->stats[STAT_MAGICPOINTS] << std::noshowpos;
			}
			...
			if (show == 0) {
				...
				s << "defense all " << std::showpos << show << std::noshowpos << '%';
			}
			...
			if (show == 0) {
				...
				s << "defense all fields " << std::showpos << show << std::noshowpos << '%';
			}
	...
	if (it.wieldInfo != 0) {
		...
		if (it.wieldInfo & WIELDINFO_MAGLV) {
			...
			s << " ki level " << it.minReqMagicLevel << " ou maior";
		}
