...
bool MoveEvents::registerEvent(Event_ptr event, const pugi::xml_node& node)
{
	...
	if ((attr = node.attribute("itemid"))) {
		std::vector<int32_t> itemIds = vectorAtoi(explodeString(attr.as_string(), ";"));
...
MoveEvent* MoveEvents::getEvent(Item* item, MoveEvent_t eventType, slots_t slot)
{
	...
	switch (slot) {
		...
		case CONST_SLOT_AMULET: slotp = SLOTP_AMULET; break;
		case CONST_SLOT_AMULET2: slotp = SLOTP_AMULET2; break;
		case CONST_SLOT_ANEL: slotp = SLOTP_ANEL; break;
		case CONST_SLOT_ANEL2: slotp = SLOTP_ANEL2; break;
...
{
    std::set<uint16_t> specificItemIdsRing = {9815, 9814, 9816, 9817, 9813};
    std::set<uint16_t> specificItemIdsAmmo = {11401};

    if (specificItemIdsRing.count(item->getID()) > 0 && slot == CONST_SLOT_RING)
    {
        std::string playerName = player->getName();
        std::string itemDescription = item->getStrAttr(ITEM_ATTRIBUTE_DESCRIPTION);

        if (itemDescription.find(playerName) == std::string::npos)
        {
            return RETURNVALUE_CANNOTUSETHISOBJECT;
        }
    }

    if (specificItemIdsAmmo.count(item->getID()) > 0 && slot == CONST_SLOT_AMMO)
    {
        std::string playerName = player->getName();
        std::string itemDescription = item->getStrAttr(ITEM_ATTRIBUTE_DESCRIPTION);
	...
	if (eventType == MOVE_EVENT_EQUIP || eventType == MOVE_EVENT_DEEQUIP) {
		...
		if (slotAttribute) {
			...
			if (tmpStr == "head") {
				...
				slot = SLOTP_AMMO;
			} else if (tmpStr == "amulet") {
				slot = SLOTP_AMULET;
			} else if (tmpStr == "amulet2") {
				slot = SLOTP_AMULET2;
			} else if (tmpStr == "anel") {
				slot = SLOTP_ANEL;
			} else if (tmpStr == "anel2") {
				slot = SLOTP_ANEL2;
			} else {
...
{
	if (!creature->compareInstance(item->getInstanceID())) {
		return 1;
	}
	...
	if (MagicField* field = item->getMagicField()) {
		...
		if (CreatureVector* creatures = tile->getCreatures()) {
			for (Creature* creature : *creatures) {
				if (creature->getInstanceID() != item->getInstanceID()) {
					continue;
				}
				field->onStepInField(creature);
			}
...
ReturnValue MoveEvent::DeEquipItem(MoveEvent*, Player* player, Item* item, slots_t slot, bool)
{
	...
	if (it.transformDeEquipTo != 0) {
		Item* newItem = g_game.transformItem(item, it.transformDeEquipTo);
		if (newItem) {
			newItem->startDecaying();
		}
	}
