...
TalkActionResult_t Spells::playerSaySpell(Player *player, std::string &words,
  ...
  if (!instantSpell || (!param.empty() && !instantSpell->getHasParam()) ||
      (param.empty() && instantSpell->getHasParam())) {
...
CombatSpell::CombatSpell(Combat *combat, bool needTarget, bool needDirection)
    : Event(&g_spells->getScriptInterface()), combat(combat),
      needDirection(needDirection), needTarget(needTarget) {}
...
bool CombatSpell::castSpell(Creature *creature) {
  if (creature->hasCondition(CONDITION_OUTFIT, 125)) {
    return false;
  }
...
bool CombatSpell::executeCastSpell(Creature *creature, const LuaVariant &var) {
  ...
  // Set _spellCaster global so doSendMagicEffect can filter by instance
  LuaScriptInterface::pushUserdata<Creature>(L, creature);
  LuaScriptInterface::setCreatureMetatable(L, -1, creature);
  lua_setglobal(L, "_spellCaster");
...
bool Spell::playerSpellCheck(Player *player) const {
  ...
  if (player->hasCondition(CONDITION_OUTFIT, 125)) {
    return false;
  }
...
bool Spell::playerRuneSpellCheck(Player *player, const Position &toPos) {
  ...
  if (range != -1 && !g_game.canThrowObjectTo(playerPos, toPos,
...
void Spell::postCastSpell(Player *player, bool finishedCast /*= true*/,
  ...
  if (finishedCast) {
    ...
    if (player->client) {
      uint32_t sendCooldown = cooldown;
      if (sendCooldown == 0) {
        sendCooldown = groupCooldown;
      }

      if (sendCooldown > 0) {
        std::ostringstream ss;
        ss << sendCooldown << "@" << getName();
        player->client->sendExtendedOpcode(204, ss.str());
      } else {
        std::cout << "[SpellBar C++] Spell " << getName()
                  << " has no cooldown to send." << std::endl;
      }
    } else {
      std::cout << "[SpellBar C++] player->client is NULL for spell: "
                << getName() << std::endl;
    }
  }
...
bool InstantSpell::canThrowSpell(const Creature *creature,
  ...
  if (fromPos.z != toPos.z ||
      (range == -1 && !g_game.canThrowObjectTo(
                          fromPos, toPos,
                          checkLineOfSight ? SightLine_CheckSightLineAndFloor
                                           : SightLine_NoCheck)) ||
      (range != -1 && !g_game.canThrowObjectTo(
                          fromPos, toPos,
                          checkLineOfSight ? SightLine_CheckSightLineAndFloor
...
bool InstantSpell::castSpell(Creature *creature) {
  ...
  bool result = internalCastSpell(creature, var);
  if (result) {
