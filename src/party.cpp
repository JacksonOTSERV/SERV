...
bool Party::passPartyLeadership(Player* player)
{
	...
	if (!g_events->eventPartyOnpassPartyLeadership(this, player)) {
		return false;
	}
...
bool Party::setSharedExperience(Player* player, bool sharedExpActive)
{
	...
	if (sharedExpActive) {
		...
		if (this->sharedExpEnabled) {
			...
			leader->sendTextMessage(MESSAGE_INFO_DESCR, "A experiência compartilhada foi ativada, mas alguns membros da sua party estão inativos.");
		}
	} else {
