...
public:
  ...
  void setDescription(std::string desc) { nameDescription = desc; }

  void setStrDescription(std::string altdesc) { altStrDesc = altdesc; }
  ...
  bool isBlockable() const { return mType->info.isBlockable; }
  ...
  static std::unordered_map<std::string, uint32_t> monsterLevelRequirements;
  static uint32_t getRequiredLevel(const std::string &monsterName);
...
private:
  ...
  int32_t extraSpellsInterval = 0;
  int64_t lastMeleeAttack = 0;
  int64_t lastAttackTime = 0;
  std::unordered_map<uint32_t, uint64_t> detectedPlayers;
  ...
  static bool pushItem(Item *item, uint32_t instanceId = 0);
  static void pushItems(Tile *tile, uint32_t instanceId = 0);
  static bool pushCreature(Creature *creature, uint32_t instanceId = 0);
  static void pushCreatures(Tile *tile, uint32_t instanceId = 0);
