...
void Container::onAddContainerItem(Item* item)
{
	...
	const uint32_t instanceId = getInstanceID();

	//send to client
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			if (tmpPlayer->getInstanceID() == instanceId) {
				tmpPlayer->sendAddContainerItem(this, item);
			}
		}
	}
	...
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			if (tmpPlayer->getInstanceID() == instanceId) {
				tmpPlayer->onAddContainerItem(item);
			}
		}
	}
}
...
void Container::onUpdateContainerItem(uint32_t index, Item* oldItem, Item* newItem)
{
	...
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			if (tmpPlayer->getInstanceID() == instanceId) {
				tmpPlayer->sendUpdateContainerItem(this, index, newItem);
			}
		}
	}
	...
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			if (tmpPlayer->getInstanceID() == instanceId) {
				tmpPlayer->onUpdateContainerItem(this, oldItem, newItem);
			}
		}
	}
}
...
void Container::onRemoveContainerItem(uint32_t index, Item* item)
{
	...
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			if (tmpPlayer->getInstanceID() == instanceId) {
				tmpPlayer->sendRemoveContainerItem(this, index);
			}
		}
	}
	...
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			if (tmpPlayer->getInstanceID() == instanceId) {
				tmpPlayer->onRemoveContainerItem(this, item);
			}
		}
	}
}
		...
		if (index == INDEX_WHEREEVER) {
			...
			for (Item* containerItem : itemlist) {
				if (containerItem != item && containerItem->equals(item) && containerItem->getItemCount() < MAX_ITEM_COUNT) {
					if (queryAdd(slotIndex++, *item, count, flags) == RETURNVALUE_NOERROR) {
						n += MAX_ITEM_COUNT - containerItem->getItemCount();
					}
				}
			}
		} else {
			...
			if (item->equals(destItem) && destItem->getItemCount() < MAX_ITEM_COUNT) {
				if (queryAdd(index, *item, count, flags) == RETURNVALUE_NOERROR) {
					n = MAX_ITEM_COUNT - destItem->getItemCount();
				}
			}
		}
...
void Container::removeThing(Thing* thing, uint32_t count)
{
	...
	if (item->isStackable() && count != item->getItemCount()) {
		uint16_t newCount = static_cast<uint16_t>(std::max<int32_t>(0, item->getItemCount() - count)); // stack >255: count é uint16
		const int32_t oldWeight = item->getWeight();
