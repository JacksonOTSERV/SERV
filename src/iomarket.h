...
class IOMarket
{
	public:
		...
		static MarketOfferList getActiveOffers2(MarketAction_t action);
		...
		static void createOffer(uint32_t playerId, MarketAction_t action, uint32_t itemId, std::string description, std::string name, uint16_t amount, uint32_t price, bool anonymous, uint16_t optionId, const char* attributes, size_t attributesSize);
		...
		static uint32_t getTotalOffers();
