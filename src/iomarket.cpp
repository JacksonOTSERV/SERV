...
MarketOfferList IOMarket::getActiveOffers(MarketAction_t action, uint16_t itemId)
{
	...
	query << "SELECT `id`, `amount`, `price`, `created`, `anonymous`, `description`, `name`, `itemtype`, `category`, `attributes`, `attributesSize`, "
		  << "(SELECT `name` FROM `players` WHERE `id` = `player_id`) AS `player_name` "
		  << "FROM `market_offers` WHERE `sale` = " << static_cast<int>(action)
		  << " AND `itemtype` = " << itemId;
	...
	do {
		...
		offer.name = result->getString("name");
		offer.category = result->getNumber<uint16_t>("category");
		offer.itemId = result->getNumber<uint16_t>("itemtype");

		unsigned long attrSize;
		offer.attributes = result->getStream("attributes", attrSize);
		offer.attributeLength = result->getNumber<uint16_t>("attributesSize");
...
// Thalles
MarketOfferList IOMarket::getActiveOffers2(MarketAction_t action)
{
	MarketOfferList offerList;
...
MarketOfferList IOMarket::getOwnOffers(MarketAction_t action, uint32_t playerId)
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `amount`, `price`, `created`, `itemtype`, `name`, `category`, `attributes`, `attributesSize`"
		  << "FROM `market_offers` WHERE `player_id` = " << playerId
		  << " AND `sale` = " << static_cast<int>(action);
...
HistoryMarketOfferList IOMarket::getOwnHistory(MarketAction_t action, uint32_t playerId)
{
	...
	std::ostringstream query;
	query << "SELECT `itemtype`, `amount`, `price`, `expires_at`, `state` "
		  << "FROM `market_history` WHERE `player_id` = " << playerId
		  << " AND `sale` = " << static_cast<int>(action);
...
void IOMarket::checkExpiredOffers()
{
	...
	query << "SELECT `id`, `amount`, `price`, `itemtype`, `player_id`, `sale` "
		  << "FROM `market_offers` WHERE `created` <= " << lastExpireDate;
...
MarketOfferEx IOMarket::getOfferByCounter(uint32_t timestamp, uint16_t counter)
{
    ...
    std::ostringstream query;
    query << "SELECT `id`, `sale`, `itemtype`, `amount`, `created`, `price`, `player_id`, `anonymous`, `name`, `category`, `attributes`, `attributesSize`, "
          << "(SELECT `name` FROM `players` WHERE `id` = `player_id`) AS `player_name` "
          << "FROM `market_offers` WHERE `id` = " << counter << " AND `created` >= " << created << " LIMIT 1";
...
void IOMarket::createOffer(uint32_t playerId, MarketAction_t action, uint32_t itemId, std::string description, std::string name, uint16_t amount, uint32_t price, bool anonymous, uint16_t optionId, const char* attributes, size_t attributesSize)
{
    std::unordered_map<uint16_t, std::vector<std::string>> categoryKeywords = {
        {1, {"armor"}},
        {2, {"boots"}},
        {3, {"hat"}},
        {4, {"legs"}},
    ...
    auto it = categoryKeywords.find(optionId);
    if (it != categoryKeywords.end()) {
        bool valid = false;
    ...
    query << "INSERT INTO `market_offers` (`id`, `player_id`, `sale`, `itemtype`, `description`, `name`, `category`, `attributes`, `attributesSize`, `amount`, `price`, `created`, `anonymous`) VALUES ("
          << '0' << ", " << playerId << ", " << action << ", " << itemId << ", " << db.escapeString(description) << ", " << db.escapeString(name) << ", " << optionId << ", " << db.escapeBlob(attributes, attributesSize) << ", " << attributesSize << ", " << amount << ", " << price << ", " << time(nullptr) << ", " << anonymous << ")";
...
void IOMarket::updateStatistics()
{
	...
	query << "SELECT `sale` AS `sale`, `itemtype` AS `itemtype`, COUNT(`price`) AS `num`, MIN(`price`) AS `min`, MAX(`price`) AS `max`, SUM(`price`) AS `sum` "
		  << "FROM `market_history` WHERE `state` = " << OFFERSTATE_ACCEPTED << " GROUP BY `itemtype`, `sale`";
	DBResult_ptr result = Database::getInstance().storeQuery(query.str());
...
uint32_t IOMarket::getTotalOffers()
{
	std::ostringstream query;
