...
static std::unordered_map<uint32_t, std::chrono::steady_clock::time_point> spectatorLastDisconnect;
static std::mutex spectatorMutex;

static bool canSpectatorReconnect(uint32_t ip) {
    std::lock_guard<std::mutex> lock(spectatorMutex);
    auto now = std::chrono::steady_clock::now();

    auto it = spectatorLastDisconnect.find(ip);
    if (it != spectatorLastDisconnect.end()) {
...
static void updateSpectatorDisconnectTime(uint32_t ip) {
    std::lock_guard<std::mutex> lock(spectatorMutex);
    spectatorLastDisconnect[ip] = std::chrono::steady_clock::now();
}
...
void Connection::close(bool force)
{
	...
	auto protoGame = std::dynamic_pointer_cast<ProtocolGame>(protocol);
	if (protoGame && protoGame->isSpectatorConnection()) {
		protoGame->forceRemoveFromSpectatorList();

		updateSpectatorDisconnectTime(getIP());
	}
