...
bool Tile::hasPropertyGlobal(const Item* exclude, ITEMPROPERTY prop) const
{
	...
	if (const TileItemVector* items = getItemList()) {
		for (const Item* item : *items) {
			if (item != exclude && item->getInstanceID() == 0 && item->hasProperty(prop)) {
				return true;
			}
		}
	}
...
MagicField* Tile::getFieldItem(uint32_t instanceID) const
{
	...
	MagicField* fallback = nullptr;
	if (const TileItemVector* items = getItemList()) {
		for (auto it = items->begin(), end = items->end(); it != end; ++it) {
			MagicField* mf = (*it)->getMagicField();
			if (!mf) {
				continue;
			}
			const uint32_t fieldInstance = (*it)->getInstanceID();
			if (fieldInstance == instanceID) {
				return mf;
			}
			// map-loaded field (instance 0) → fallback if no exact match
			if (fieldInstance == 0 && (*it)->isLoadedFromMap()) {
				fallback = mf;
			}
		}
	}
...
Thing* Tile::getTopVisibleThing(const Creature* creature)
{
	...
	const uint32_t viewerInstanceId = creature ? creature->getInstanceID() : 0;
	...
	if (const Creature* creature = thing.getCreature()) {
		...
		if (const Monster* monster = creature->getMonster()) {
			...
			if (monster->canPushCreatures() && !monster->isSummon()) {
				if (creatures) {
					for (Creature* tileCreature : *creatures) {
						if (tileCreature->getInstanceID() != creature->getInstanceID()) {
							continue;
						}
		...
		if (creatures && !creatures->empty() && item->isBlocking() && !hasBitSet(FLAG_IGNOREBLOCKCREATURE, flags)) {
			for (const Creature* tileCreature : *creatures) {
				if (tileCreature->getInstanceID() != thing.getInstanceID()) {
					continue;
				}
				if (!tileCreature->isInGhostMode()) {
					return RETURNVALUE_NOTENOUGHROOM;
...
void Tile::removeThing(Thing* thing, uint32_t count)
{
	...
	if (itemType.alwaysOnTop) {
		...
		if (itemType.stackable && count != item->getItemCount()) {
			uint16_t newCount = static_cast<uint16_t>(std::max<int32_t>(0, static_cast<int32_t>(item->getItemCount() - count))); // stack >255: count é uint16
...
int32_t Tile::getClientIndexOfCreature(const Player* player, const Creature* creature) const
{
	...
	const uint32_t viewerInstance = player->getInstanceID();
	const TileItemVector* items = getItemList();
	if (items) {
		// Must match GetTileDescription filtering — count only instance-visible topItems
		for (auto it = items->getBeginTopItem(), end = items->getEndTopItem(); it != end; ++it) {
			if (InstanceUtils::canSeeItemInInstance(viewerInstance, *it)) {
				++n;
			}
		}
	}
...
void Tile::postAddNotification(Thing* thing, const Cylinder* oldParent, int32_t index, cylinderlink_t link /*= LINK_OWNER*/)
{
	...
	const uint32_t thingInstanceId = thing->getInstanceID();
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			if (tmpPlayer->getInstanceID() == thingInstanceId || thingInstanceId == 0) {
...
void Tile::postRemoveNotification(Thing* thing, const Cylinder* newParent, int32_t index, cylinderlink_t)
{
	...
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			if (tmpPlayer->getInstanceID() == thingInstanceId || thingInstanceId == 0) {
				tmpPlayer->postRemoveNotification(thing, newParent, index, LINK_NEAR);
			}
		}
	}
