...
public:
  MatrixArea(uint32_t rows, uint32_t cols)
      : centerX(0), centerY(0), rows(rows), cols(cols) {
    data_ = new bool *[rows];
...
private:
  ...
  static void copyArea(const MatrixArea *input, MatrixArea *output,
...
public:
  ...
  static void registerAttacker(Creature *target, Player *attacker);

  static std::vector<Player *> getAttackers(Creature *target);
  ...
  // PowerUp System helpers - applied in all combat paths
  static void applyOffensivePowerUps(Player *casterPlayer, Creature *target,
                                     CombatDamage &damage);
  static void applyLeechPowerUps(Player *casterPlayer,
                                 const CombatDamage &damage);

  static void doTargetCombat(Creature *caster, Creature *target,
                             CombatDamage &damage, const CombatParams &params);
  static void doAreaCombat(Creature *caster, const Position &position,
                           const AreaCombat *area, CombatDamage &damage,
                           const CombatParams &params);
...
private:
  struct AttackerInfo {
    Player *player;
    uint64_t lastHit; // timestamp do último ataque
  };

  static std::unordered_map<Creature *, std::vector<AttackerInfo>> attackersMap;
  ...
  static void combatTileEffects(const SpectatorVector &spectators,
