...
bool Events::load()
{
	...
	for (auto eventNode : doc.child("events").children()) {
		...
		if (className == "Creature") {
			...
			} else if (methodName == "onpassPartyLeadership") {
				info.partyOnpassPartyLeadership = event;
			} else {
...
bool Events::eventPartyOnpassPartyLeadership(Party* party, Player* player)
{
	if (info.partyOnpassPartyLeadership == -1) {
		return true;
	}
	...
	ScriptEnvironment* env = scriptInterface.getScriptEnv();
	env->setScriptId(info.partyOnpassPartyLeadership, &scriptInterface);

	lua_State* L = scriptInterface.getLuaState();
	scriptInterface.pushFunction(info.partyOnpassPartyLeadership);
...
bool Events::eventPlayerOnUseItem(Player* player, Item* item, const Position& fromPosition, Thing* target, const Position& toPosition, bool isHotkey)
{
    ...
    if (target) {
        if (Creature* creatureTarget = target->getCreature()) {
            LuaScriptInterface::pushUserdata<Creature>(L, creatureTarget);
            LuaScriptInterface::setCreatureMetatable(L, -1, creatureTarget);
        } else if (Item* itemTarget = target->getItem()) {
            LuaScriptInterface::pushUserdata<Item>(L, itemTarget);
            LuaScriptInterface::setItemMetatable(L, -1, itemTarget);
        } else {
            lua_pushnil(L);
        }
    } else {
