...
void Map::moveCreature(Creature& creature, Tile& newTile, bool forceTeleport/* = false*/)
{
	...
	if (!teleport) {
		// tactical broadcast: expandido pra 50sqm (creature move visible em raio maior)
		int32_t minRangeX = tacticalRangeX;
		int32_t maxRangeX = tacticalRangeX;
		int32_t minRangeY = tacticalRangeY;
		int32_t maxRangeY = tacticalRangeY;
	...
	//send to client — only to spectators in the same instance
	const uint32_t creatureInstance = creature.Thing::getInstanceID();
	size_t i = 0;
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			//Use the correct stackpos
			int32_t stackpos = oldStackPosVector[i++];
			if (stackpos != -1 && tmpPlayer->Thing::getInstanceID() == creatureInstance) {
				tmpPlayer->sendCreatureMove(&creature, newPos, newTile.getStackposOfCreature(tmpPlayer, &creature), oldPos, stackpos, teleport);
			}
		}
	}

	//event method — only notify spectators in the same instance
	for (Creature* spectator : spectators) {
		if (spectator->Thing::getInstanceID() == creatureInstance) {
...
bool Map::checkSightLine(const Position& fromPos, const Position& toPos) const
{
    ...
    while (start.x != destination.x || start.y != destination.y) {
        ...
        if (tile) {
            ...
            if (items) {
                for (auto& item : *items) {
                    if (item->getID() == 1228 || item->getID() == 1230 || item->getID() == 5104 || item->getID() == 11398) {
                        return false;
                    }
                }
            }
