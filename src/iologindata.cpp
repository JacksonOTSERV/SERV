...
void IOLoginData::updateOnlineStatus(uint32_t guid, bool login)
{
    ...
    if (login) {
        ...
        checkQuery << "SELECT COUNT(*) FROM players_online WHERE player_id = " << guid;

        DBResult_ptr result = Database::getInstance().storeQuery(checkQuery.str());
        if (result && result->getNumber<uint32_t>("COUNT(*)") > 0) {
            return; // Já está na tabela, não insere novamente
        }
...
bool IOLoginData::loadPlayerById(Player* player, uint32_t id)
{
	Database& db = Database::getInstance();
	std::ostringstream query;
	query << "SELECT `id`, `name`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `skulltime`, `skull`, `town_id`, `balance`, `stamina`, `skill_fist`, `skill_fist_tries`, `skill_club`, `skill_club_tries`, `skill_sword`, `skill_sword_tries`, `skill_axe`, `skill_axe_tries`, `skill_dist`, `skill_dist_tries`, `skill_shielding`, `skill_shielding_tries`, `skill_fishing`, `skill_fishing_tries`, `skill_pesca`, `skill_pesca_tries`, `direction` FROM `players` WHERE `id` = " << id;
...
bool IOLoginData::loadPlayerByName(Player* player, const std::string& name)
{
	Database& db = Database::getInstance();
	std::ostringstream query;
	query << "SELECT `id`, `name`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `skulltime`, `skull`, `town_id`, `balance`, `stamina`, `skill_fist`, `skill_fist_tries`, `skill_club`, `skill_club_tries`, `skill_sword`, `skill_sword_tries`, `skill_axe`, `skill_axe_tries`, `skill_dist`, `skill_dist_tries`, `skill_shielding`, `skill_shielding_tries`, `skill_fishing`, `skill_fishing_tries`, `skill_pesca`, `skill_pesca_tries`, `direction` FROM `players` WHERE `name` = " << db.escapeString(name);
	return loadPlayer(player, db.storeQuery(query.str()));
}
...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	if (g_game.getWorldType() != WORLD_TYPE_PVP_ENFORCED) {
		...
		std::ostringstream query;
		query << "SELECT COUNT(*) AS total FROM player_frags WHERE killer_id = " << player->getGUID()
			  << " AND (frag_time + " << fragDuration << ") > " << now;

		DBResult_ptr result = db.storeQuery(query.str());
		if (result) {
			uint32_t totalFrags = result->getNumber<uint32_t>("total");

			uint32_t killsToBlack = static_cast<uint32_t>(g_config.getNumber(ConfigManager::KILLS_TO_BLACK));
			uint32_t killsToRed = static_cast<uint32_t>(g_config.getNumber(ConfigManager::KILLS_TO_RED));
	...
	static const std::string skillNames[] = {"skill_fist", "skill_club", "skill_sword", "skill_axe", "skill_dist", "skill_shielding", "skill_fishing", "skill_pesca"};
	static const std::string skillNameTries[] = {"skill_fist_tries", "skill_club_tries", "skill_sword_tries", "skill_axe_tries", "skill_dist_tries", "skill_shielding_tries", "skill_fishing_tries", "skill_pesca_tries"};
...
bool IOLoginData::saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert, PropWriteStream& propWriteStream, std::map<Container*, int>& openContainers)
{
    ...
    // Função auxiliar para extrair a parte da descrição que contém "Chance to apply"
    auto extractChanceDescription = [](const std::string& description) -> std::string {
        size_t pos = description.find("Chance to apply");
        if (pos != std::string::npos) {
            // Encontrou "Chance to apply", retorna a parte da descrição a partir dessa frase
            return description.substr(pos);  // Retorna apenas a parte relevante da descrição
        }
        return "";  // Se não encontrar, retorna uma string vazia
    };

    // Iterando pelos itens na lista
    for (const auto& it : itemList) {
        ...
        std::string itemDescription = item->getStrAttr(ITEM_ATTRIBUTE_DESCRIPTION);

        // Processar a descrição: Se houver "Chance to apply", salva apenas essa parte, caso contrário, salva em branco
        std::string filteredDescription = extractChanceDescription(itemDescription);

        ss << player->getGUID() << ',' << pid << ',' << runningId << ',' << item->getID() << ',' << item->getSubType() << ',' 
           << db.escapeString(itemName) << ',' << db.escapeString(filteredDescription) << ',' 
...
bool IOLoginData::saveItemsTwo(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert, PropWriteStream& propWriteStream, std::map<Container*, int>& openContainers)
{
    std::ostringstream ss;
    ...
    for (const auto& it : itemList) {
        ...
        // Retrieve the item name from item properties or predefined list
        std::string itemName = item->getName(); // You need to ensure this method exists or replace it with appropriate logic

        ss << player->getGUID() << ',' << pid << ',' << runningId << ',' << item->getID() << ',' << item->getSubType() << ',' << db.escapeString(itemName) << ',' << db.escapeBlob(attributes, attributesSize);
        if (!query_insert.addRow(ss)) {
            return false;
        }
    }
    ...
    for (size_t i = 0; i < containers.size(); i++) {
        ...
        for (Item* item : container->getItemList()) {
            ...
            ss << player->getGUID() << ',' << parentId << ',' << runningId << ',' << item->getID() << ',' << item->getSubType() << ',' << db.escapeString(itemName) << ',' << db.escapeBlob(attributes, attributesSize);
            if (!query_insert.addRow(ss)) {
                return false;
            }
        }
...
bool IOLoginData::savePlayer(Player* player)
{
	...
	query << "`skill_pesca` = " << player->skills[SKILL_PESCA].level << ',';
	query << "`skill_pesca_tries` = " << player->skills[SKILL_PESCA].tries << ',';
	...
	DBInsert itemsQuery("INSERT INTO `player_items` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `item_name`, `item_description`, `attributes`) VALUES ");

	ItemBlockList itemList;
	for (int32_t slotId = 1; slotId <= 15; ++slotId) {
	...
	if (player->lastDepotId != -1) {
		...
		DBInsert depotQuery("INSERT INTO `player_depotitems` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `item_name`, `attributes`) VALUES ");
		itemList.clear();
		...
		if (!saveItemsTwo(player, itemList, depotQuery, propWriteStream, openContainers)) {
			return false;
		}
	}
	...
	DBInsert inboxQuery("INSERT INTO `player_inboxitems` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `item_name`, `attributes`) VALUES ");
	itemList.clear();
	...
	if (!saveItemsTwo(player, itemList, inboxQuery, propWriteStream, openContainers)) {
		return false;
	}
	...
	if (!rewardList.empty()) {
		DBInsert rewardQuery("INSERT INTO `player_rewards` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `item_name`, `attributes`) VALUES ");
		itemList.clear();
		...
		if (!saveItemsTwo(player, itemList, rewardQuery, propWriteStream, openContainers)) {
			return false;
		}
	}
...
uint32_t IOLoginData::getPremiumPoints(uint32_t accountId)
{
	std::ostringstream query;
	...
	DBResult_ptr result = Database::getInstance().storeQuery(query.str());
	if (result) {
		points = result->getNumber<uint32_t>("premium_points");
	}
	return points;
}

void IOLoginData::setPremiumPoints(uint32_t accountId, uint32_t points)
{
	std::ostringstream query;
	query << "UPDATE `accounts` SET `premium_points` = " << points << " WHERE `id` = " << accountId;
	Database::getInstance().executeQuery(query.str());
}