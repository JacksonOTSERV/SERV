...
class Game
{
    ...
    uint8_t getSpawnRate() const;
		...
		void playerToggleOutfitExtension(uint32_t playerId, int wings, int aura);
		void playerLeaveMarket(uint32_t playerId);
		void playerBrowseMarket(uint32_t playerId, uint16_t spriteId);
		void playerBrowseMarket2(uint32_t playerId);
		void playerUpdateMarketEnter(uint32_t playerId);
		void playerBrowseMarketOwnOffers(uint32_t playerId);
		void playerBrowseMarketOwnHistory(uint32_t playerId);
		void playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t spriteId, uint16_t amount, uint32_t price, bool anonymous, uint16_t optionId);
		...
		void updateCreatureOutfit(Creature* creature);
		...
		void addAnimatedText(const std::string& message, const Position& pos, TextColor_t color, uint32_t instanceId = 0);
		...
		static void addMagicEffect(const SpectatorVector& spectators, const Position& pos, uint16_t effect);
		...
		static void addDistanceEffect(const SpectatorVector& spectators, const Position& fromPos, const Position& toPos, uint16_t effect);
		...
		int64_t getDecayTime(const Item* item);
		void startDecay(Item* item);
		void stopDecay(Item* item);
		void updateDecay(Item* item, int64_t time);
	...
	private:
		...
		// decay
		std::map<int64_t, std::map<Item*, Item*>> itemDecayMap;
		std::unordered_map<const Item*, int64_t> reverseItemDecayMap;
	...
	public:
		Map map; // definido por ultimo para ser o primeiro a ser deletado
