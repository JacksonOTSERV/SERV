...
bool Protocol::compress(OutputMessage& msg) const
{
	...
	int finalSize = static_cast<int>(buffer.size()) - static_cast<int>(zstream.avail_out) - 4;
	if (finalSize <= 0) {
		std::cerr << "Packet compression error: finalSize=" << finalSize << std::endl;
		return false;
	}
