...
const std::unordered_map<std::string, ItemParseAttributes_t> ItemParseAttributesMap = {
	...
	{"skillpesca", ITEM_PARSE_SKILLPESCA},
...
void Items::buildInventoryList()
{
    ...
    for (const auto& type: items) {
        if (type.weaponType != WEAPON_NONE || type.ammoType != AMMO_NONE ||
            ...
            type.slotPosition & SLOTP_AMULET ||
            type.slotPosition & SLOTP_AMULET2 ||
            type.slotPosition & SLOTP_ANEL ||
            type.slotPosition & SLOTP_ANEL2)
        {
			...
			switch (parseType) {
				...
				case ITEM_PARSE_SLOTTYPE: {
					...
					if (tmpStrValue == "head") {
						...
						it.slotPosition |= SLOTP_AMULET;
					} else if (tmpStrValue == "amulet2") {
						it.slotPosition |= SLOTP_AMULET2;
					} else if (tmpStrValue == "anel") {
						it.slotPosition |= SLOTP_ANEL;
					} else if (tmpStrValue == "anel2") {
						it.slotPosition |= SLOTP_ANEL2;
					} else {
				...
				case ITEM_PARSE_SKILLPESCA: {
                    abilities.skills[SKILL_PESCA] = pugi::cast<int32_t>(valueAttribute.value());
                    break;
                }
