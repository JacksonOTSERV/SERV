...
public:
  void forceRemoveFromSpectatorList();
  bool isSpectatorConnection() const;
   ...
   *  \param spectatorClient pointer to the \ref ProtocolSpectator object
   ...
   */
  bool stopLiveCast();
  const CastSpectatorVec &getLiveCastSpectators() const { return spectators; }
  size_t getSpectatorCount() const { return spectators.size(); }
  bool isLiveCaster() const { return isCaster.load(std::memory_order_relaxed); }
  ...
  /** \brief Clears all live casts. Used to make sure there aro no live cast db
   * rows left should a crash occur.
  ...
  const std::string &getLiveCastName() const { return liveCastName; }
  const std::string &getLiveCastPassword() const { return liveCastPassword; }
  bool isPasswordProtected() const { return !liveCastPassword.empty(); }
...
private:
  ...
  std::atomic<bool> isCaster{
      false}; ///< Determines if this \ref ProtocolGame object is casting
  /// list of spectators \warning This variable should only be accessed after
  /// locking \ref liveCastLock
