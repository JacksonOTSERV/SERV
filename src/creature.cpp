...
{
	return canSee(getPosition(), pos, 15, 8);
}

bool Creature::canSeeCreature(const Creature* creature) const
{
	// Different instances can't see each other, UNLESS the target is
	// a map-resident creature (instance 0) — NPCs/vendors placed on the map.
	if (Thing::getInstanceID() != creature->Thing::getInstanceID()
	    && creature->Thing::getInstanceID() != 0) {
		return false;
	}
...
int32_t Creature::getWalkDelay() const
{
	...
	int64_t elapsed = ct - (int64_t)lastStep;
	return (int32_t)(stepDuration - elapsed);
}
...
void Creature::onWalk(Direction& dir)
{
    if (hasCondition(CONDITION_DRUNK)) {
        ...
        g_game.addMagicEffect(this->getPosition(), 92);
    }
}
...
void Creature::addEventWalk(bool firstStep)
{
	...
	uint32_t cid = getID();
	auto task = std::make_unique<SchedulerTask>(ticks, [cid]() {
		g_game.checkCreatureWalk(cid);
	});
	eventWalk = g_scheduler.addEvent(std::move(task));
}
                              ...
                              const Tile* oldTile, const Position& oldPos, bool teleport)
{
	if (creature == this) {
		...
		// Player Trails — spawn magic effect na oldPos (instant, sem polling)
		// Storage key 87651 = trail id ativo (lua side). 0 = off.
		if (!teleport && oldPos.z == newPos.z) {
			Player* p = creature->getPlayer();
			if (p) {
				int32_t trailId = 0;
				p->getStorageValue(87651, trailId);
				if (trailId > 0) {
					static const uint16_t TRAIL_EFFECTS[] = {
						0,    // [0] unused
						1978, // [1] Default custom
						CONST_ME_HITBYFIRE,    // [2] Red
						CONST_ME_MAGIC_BLUE,   // [3] Blue
						CONST_ME_MAGIC_GREEN,  // [4] Green
					};
					if (trailId >= 1 && trailId < (int32_t)(sizeof(TRAIL_EFFECTS) / sizeof(uint16_t))) {
						g_game.addMagicEffect(oldPos, TRAIL_EFFECTS[trailId]);
					}
				}
			}
		}
...
bool Creature::dropCorpse(Creature* lastHitCreature, Creature* mostDamageCreature, bool lastHitUnjustified, bool mostDamageUnjustified)
{
	if (!lootDrop && getMonster()) {
		...
		if (splash) {
			splash->setInstanceID(Thing::getInstanceID());
		...
		if (corpse) {
			corpse->setInstanceID(Thing::getInstanceID());
                               ...
                               bool checkDefense /* = false */, bool checkArmor /* = false */, bool /* field  = false */, bool /* ignoreResistances = false */)
{
	...
	if (isImmune(combatType)) {
		...
		if (checkArmor) {
			...
			int32_t minArmorReduction = 0, maxArmorReduction = 0;
...
{
	const MagicField* field = getTile()->getFieldItem(Thing::getInstanceID());
	if (!field) {
		return;
	}
...
void Creature::sendDodge() const
{
	g_game.addAnimatedText("Dodged!", getPosition(), TEXTCOLOR_WHITE, getInstanceID());
}

void Creature::sendCritical() const
{
    if (g_config.getBoolean(ConfigManager::DISPLAY_CRITICAL_HIT)) {
        g_game.addAnimatedText("Critic!", getPosition(), TEXTCOLOR_RED, getInstanceID());
    }
}

void Creature::insertAttachedEffect(uint16_t effectId)
{
	attachedEffects[effectId] = { effectId };
	g_game.updateCreatureOutfit(this);
}
