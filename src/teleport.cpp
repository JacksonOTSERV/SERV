...
void Teleport::addThing(int32_t, Thing* thing)
{
	...
	if (Creature* creature = thing->getCreature()) {
		...
		if (effect != CONST_ME_NONE) {
			g_game.addMagicEffect(origPos, effect, instanceId);
			g_game.addMagicEffect(destTile->getPosition(), effect, instanceId);
		}
	} else if (Item* item = thing->getItem()) {
		const uint32_t instanceId = item->getInstanceID();
		if (effect != CONST_ME_NONE) {
			g_game.addMagicEffect(destTile->getPosition(), effect, instanceId);
			g_game.addMagicEffect(item->getPosition(), effect, instanceId);
		}
