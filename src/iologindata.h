...
class IOLoginData
{
	public:
		...
		// Thalles Vitor
		static uint32_t getPremiumPoints(uint32_t accountId);
		static void setPremiumPoints(uint32_t accountId, uint32_t points);
	...
	private:
		...
		static bool saveItemsTwo(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert, PropWriteStream& propWriteStream, std::map<Container*, int>& openContainers);
};
