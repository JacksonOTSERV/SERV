...
std::unordered_map<Creature *, std::vector<Combat::AttackerInfo>>
    Combat::attackersMap;

void Combat::registerAttacker(Creature *target, Player *attacker) {
  if (!target || !attacker)
    return;
  if (!target->getPlayer() || attacker == target)
    return;

  uint64_t now = OTSYS_TIME();

  auto &list = attackersMap[target];
  bool found = false;
  for (auto &info : list) {
  ...
  if (!found) {
    list.push_back({attacker, now});
  }
}

std::vector<Player *> Combat::getAttackers(Creature *target) {
  std::vector<Player *> result;
  if (!target)
    return result;

  uint64_t now = OTSYS_TIME();
  auto it = attackersMap.find(target);
  if (it != attackersMap.end()) {
    auto &list = it->second;
    list.erase(std::remove_if(list.begin(), list.end(),
                              [&](const AttackerInfo &info) {
                                return now - info.lastHit > 1500;
                              }),
               list.end());

    for (const auto &info : list) {
      if (info.player) {
        result.push_back(info.player);
      }
    }
  }
...
CombatDamage Combat::getCombatDamage(Creature *creature,
  ...
  if (formulaType == COMBAT_FORMULA_DAMAGE) {
    ...
    if (creature->getCombatValues(min, max)) {
      ...
      if (params.valueCallback) {
        ...
        float magDamageMultiplier = vocation ? vocation->getMagDamage() : 2.7f;

        int32_t levelFormula =
            player->getLevel() * 2 + player->getMagicLevel() * 3;
        constexpr float minMultiplier = 0.8f;
        constexpr float maxMultiplier = 1.2f;

        int32_t minDamage = static_cast<int32_t>(
            levelFormula * magDamageMultiplier * minMultiplier);
        int32_t maxDamage = static_cast<int32_t>(
            levelFormula * magDamageMultiplier * maxMultiplier);
        ...
        if (weapon) {
          damage.primary.value = normal_random(
              minb,
              std::fma(weapon->getWeaponDamage(player, target, tool, true),
                       maxa, maxb));
  ...
  if (creature) {
    std::string attackerName = creature->getName();
    if (attackerName.find("lvl.2") != std::string::npos) {
      damage.primary.value = static_cast<int32_t>(damage.primary.value * 1.15f);
      damage.secondary.value =
          static_cast<int32_t>(damage.secondary.value * 1.15f);
    }
  }
  return damage;
}
...
void Combat::getCombatArea(const Position &centerPos, const Position &targetPos,
                           const AreaCombat *area, std::vector<Tile *> &list) {
...
ReturnValue Combat::canTargetCreature(Player *attacker, Creature *target) {
  ...
  if (target->getPlayer()) {
    ...
    if (attacker->getVocationId() != 3 && attacker->hasSecureMode() &&
...
void Combat::setPlayerCombatValues(formulaType_t formulaType, double mina,
...
void Combat::CombatHealthFunc(Creature *caster, Creature *target,
  ...
  // Apply offensive PowerUps (Strength, Critical, Healing Boost)
  if (casterPlayer) {
    applyOffensivePowerUps(casterPlayer, target, damage);
  }
  ...
  if (g_game.combatChangeHealth(caster, target, damage)) {
    ...
    // Apply Leech PowerUps after successful damage
    if (casterPlayer) {
      applyLeechPowerUps(casterPlayer, damage);
    }
  }
}
...
void Combat::combatTileEffects(const SpectatorVector &spectators,
  ...
  if (params.itemId != 0) {
    ...
    if (caster) {
      ...
      item->setSpecialDescription(caster->getName());
  ...
  if (params.impactEffect != CONST_ME_NONE) {
    const uint32_t effectInstance = caster ? caster->getInstanceID() : 0;
    SpectatorVec filtered = InstanceUtils::filterByInstance(spectators, effectInstance);
...
void Combat::doCombat(Creature *caster, Creature *target) const {
  if (caster && target) {
    ...
    if (attackerPlayer && targetPlayer) {
      Combat::registerAttacker(target, attackerPlayer);
    }
  }
  ...
  if (params.combatType != COMBAT_NONE) {
    ...
    bool canCombat = !params.aggressive ||
                     (caster != target && Combat::canDoCombat(caster, target) ==
                                              RETURNVALUE_NOERROR);
    if ((caster == target || canCombat) &&
        params.impactEffect != CONST_ME_NONE) {
      const uint32_t effectInstance = target ? target->getInstanceID() : (caster ? caster->getInstanceID() : 0);
      g_game.addMagicEffect(target->getPosition(), params.impactEffect, effectInstance);
    }
...
void Combat::doCombat(Creature *caster, const Position &position) const {
  ...
  if (params.combatType != COMBAT_NONE) {
    ...
    SpectatorVector spectators;
    g_game.map.getSpectators(spectators, position, true, true, rangeX, rangeX,
                             rangeY, rangeY);
      ...
      if (CreatureVector *creatures = tile->getCreatures()) {
        ...
        for (Creature *creature : *creatures) {
          ...
          if (caster && creature) {
            ...
            if (attackerPlayer && targetPlayer) {
              Combat::registerAttacker(creature, attackerPlayer);
            }
          }
...
// ==========================================
// POWERUP SYSTEM: Helper Functions
// Called from doTargetCombat, CombatHealthFunc, and doAreaCombat
// ==========================================
void Combat::applyOffensivePowerUps(Player *casterPlayer, Creature *target,
                                    CombatDamage &damage) {
  if (!casterPlayer)
    return;

  // Healing Boost (storage 50005)
  if (damage.primary.type == COMBAT_HEALING ||
      damage.secondary.type == COMBAT_HEALING) {
    int32_t healingBoost = 0;
    if (casterPlayer->getStorageValue(50005, healingBoost) &&
        healingBoost > 0) {
      std::cout << "[PowerUp] Healing Boost: " << healingBoost << "%"
                << std::endl;
      if (damage.primary.type == COMBAT_HEALING)
        damage.primary.value +=
            std::round(damage.primary.value * (healingBoost / 100.0));
      if (damage.secondary.type == COMBAT_HEALING)
        damage.secondary.value +=
            std::round(damage.secondary.value * (healingBoost / 100.0));
    }
    return; // Healing spells don't get Strength/Critical
  }

  // Strength (storage 50003) - increases damage
  int32_t strength = 0;
  if (casterPlayer->getStorageValue(50003, strength) && strength > 0) {
    std::cout << "[PowerUp] Strength: " << strength << "% on dmg "
              << damage.primary.value << "/" << damage.secondary.value
              << std::endl;
    damage.primary.value +=
        std::round(damage.primary.value * (strength / 100.0));
    damage.secondary.value +=
        std::round(damage.secondary.value * (strength / 100.0));
  }

  // Critical Hit (storage 50006 = chance, 50007 = bonus)
  int32_t critChance = 0;
  if (casterPlayer->getStorageValue(50006, critChance) && critChance > 0) {
    if (normal_random(1, 100) <= critChance) {
      int32_t critDmg = 0;
      casterPlayer->getStorageValue(50007, critDmg);
      float mult = 2.0f + (critDmg / 100.0f);

      std::cout << "[PowerUp] CRITICAL! Chance: " << critChance
                << "%, Bonus: " << critDmg << "%, Mult: " << mult << std::endl;

      damage.primary.value = static_cast<int32_t>(damage.primary.value * mult);
      damage.secondary.value =
          static_cast<int32_t>(damage.secondary.value * mult);
      casterPlayer->sendTextMessage(MESSAGE_STATUS_SMALL, "CRITICAL!");
      if (target) {
        g_game.addMagicEffect(target->getPosition(), CONST_ME_MORTAREA, target->getInstanceID());
      }
    }
  }
}

void Combat::applyLeechPowerUps(Player *casterPlayer,
  ...
  int32_t totalDamage =
      std::abs(damage.primary.value) + std::abs(damage.secondary.value);
  if (totalDamage <= 0)
    return;

  // Life Leech (storage 50008)
  int32_t lifeLeech = 0;
  if (casterPlayer->getStorageValue(50008, lifeLeech) && lifeLeech > 0) {
    int32_t heal = static_cast<int32_t>(totalDamage * (lifeLeech / 100.0f));
    if (heal > 0) {
      std::cout << "[PowerUp] Life Leech: +" << heal << " HP" << std::endl;
  ...
  // Mana Leech (storage 50009)
  int32_t manaLeech = 0;
  if (casterPlayer->getStorageValue(50009, manaLeech) && manaLeech > 0) {
    int32_t mana = static_cast<int32_t>(totalDamage * (manaLeech / 100.0f));
    if (mana > 0) {
      std::cout << "[PowerUp] Mana Leech: +" << mana << " MP" << std::endl;
      casterPlayer->changeMana(mana);
      casterPlayer->sendMagicEffect(casterPlayer->getPosition(),
                                    CONST_ME_MAGIC_BLUE);
    }
  }
...
// ==========================================
// doTargetCombat - now uses helper functions
// ==========================================
...
void Combat::doAreaCombat(Creature *caster, const Position &position,
  ...
  for (Tile *tile : tileList) {
    ...
    if (CreatureVector *creatures = tile->getCreatures()) {
      ...
      for (Creature *creature : *creatures) {
        ...
        if (caster) {
          uint32_t requiredLevel = Monster::getRequiredLevel(caster->getName());
          Player *targetPlayer = creature->getPlayer();
          if (targetPlayer && targetPlayer->getLevel() < requiredLevel) {
            continue;
          }
        }
        ...
        if (!params.aggressive ||
          ...
          CombatDamage damageCopy =
              damage; // we cannot avoid copying here, because we don't know
                      // if it's player combat or not, so we can't modify
                      // the initial damage.

          // Apply offensive PowerUps (Strength, Critical, Healing Boost)
          Player *casterPlayer = caster ? caster->getPlayer() : nullptr;
          if (casterPlayer) {
            applyOffensivePowerUps(casterPlayer, creature, damageCopy);
          }

          if ((damageCopy.primary.value < 0 ||
               damageCopy.secondary.value < 0) &&
              caster) {
            Player *targetPlayer = creature->getPlayer();
          ...
          if (damageCopy.primary.type != COMBAT_MANADRAIN) {
            if (g_game.combatBlockHit(
                    damageCopy, caster, creature, params.blockedByShield,
                    params.blockedByArmor, params.itemId != 0)) {
              continue;
            }
          ...
          if (success) {
            if (params.origin != ORIGIN_MELEE &&
                damageCopy.primary.value != 0 &&
                damageCopy.secondary.value != 0) {
              for (const auto &condition : params.conditionList) {
                if (caster == creature ||
                    !creature->isImmune(condition->getType())) {
                  Condition *conditionCopy = condition->clone();
                  conditionCopy->setParam(CONDITION_PARAM_OWNER,
                                          caster->getID());
                  creature->addCombatCondition(conditionCopy);
                }
              }
            ...
            // Apply Leech PowerUps after successful damage
            if (casterPlayer) {
              applyLeechPowerUps(casterPlayer, damageCopy);
            }
          }
...
void ValueCallback::getMinMaxValues(Player *player,
                                    CombatDamage &damage) const {
  ...
  case COMBAT_FORMULA_SKILL: {
    // onGetPlayerMinMaxValues(player, attackSkill, attackValue,
    // attackFactor)
    Item *tool = player->getWeapon();
    const Weapon *weapon = g_weapons->getWeapon(tool);
    Item *item = nullptr;
