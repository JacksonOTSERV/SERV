...
bool ConditionAttributes::setParam(ConditionParam_t param, int32_t value)
{
	...
	switch (param) {
		...
		case CONDITION_PARAM_SKILL_PESCA: {
            skills[SKILL_PESCA] = value;
            return true;
        }

        case CONDITION_PARAM_SKILL_PESCAPERCENT: {
            skillsPercent[SKILL_PESCA] = value;
            return true;
        }
	...
	if (internalHealthTicks >= healthTicks) {
		...
		if (isBuff && realHealthGain > 0) {
			...
			if (player) {
				...
				TextMessage message(MESSAGE_STATUS_DEFAULT, "Voc� foi curado por " + healString);
				...
				if (!spectators.empty()) {
					...
					for (Creature* spectator : spectators) {
						...
						if (tmpPlayer && tmpPlayer->compareInstance(playerInstanceId)) {
	...
	if (internalManaTicks >= manaTicks) {
		...
		if (Player* player = creature->getPlayer()) {
			...
			if (isBuff && realManaGain > 0) {
				...
				TextMessage message(MESSAGE_STATUS_DEFAULT, "Voc� ganhou " + manaGainString + " mana.");
				player->sendTextMessage(message);
...
bool ConditionDamage::doDamage(Creature* creature, int32_t healthChange)
{
	...
	if (!creature->isAttackable() || Combat::canDoCombat(attacker, creature) != RETURNVALUE_NOERROR) {
		if (!creature->isInGhostMode()) {
			g_game.addMagicEffect(creature->getPosition(), CONST_ME_POFF, creature->getInstanceID());
		}
		return false;
	}
...
bool ConditionOutfit::startCondition(Creature* creature)
{
    ...
    const Outfit_t& currentOutfit = creature->getCurrentOutfit();

    Outfit_t newOutfit = currentOutfit;
    ...
    g_game.internalCreatureChangeOutfit(creature, newOutfit);
    return true;
}
...
{
    const Outfit_t& defaultOutfit = creature->getDefaultOutfit();
    Outfit_t newOutfit = defaultOutfit;

    if (creature->hasCondition(CONDITION_ATTRIBUTES, 124)) {
        newOutfit.lookAura = defaultOutfit.lookAura;
    } else {
        newOutfit.lookAura = 0;
    }

    newOutfit.healthBackground = defaultOutfit.healthBackground;
    newOutfit.manaBackground = defaultOutfit.manaBackground;
    newOutfit.koteElemental = defaultOutfit.koteElemental;
    newOutfit.lookWings = defaultOutfit.lookWings;
