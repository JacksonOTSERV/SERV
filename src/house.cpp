...
void House::updateDoorDescription() const
{
    ...
    if (owner != 0) {
        ss << "Essa ? a casa '" << houseName << "'. " << ownerName << " ? o dono desta casa.";
    } else {
        ss << "Essa ? a casa '" << houseName << "'. Ningu?m ? o dono desta casa.";

        const int32_t housePrice = g_config.getNumber(ConfigManager::HOUSE_PRICE);
        if (housePrice != -1) {
            ss << " Isso custa " << (houseTiles.size() * housePrice) << " gold bars (" << ((houseTiles.size() * housePrice) / 2) << " para jogadores premium).";
        }
    }
	...
	for (auto& it : houseMap) {
		...
		if (player.getBankBalance() >= rent) {
			...
			if (house->getPayRentWarnings() < 7) {
				...
				std::ostringstream ss;
				ss << "Cuidado! \nO " << period << " aluguel de " << house->getRent() << " gold da sua casa \"" << house->getName() << "\" deve ser pago dentro de " << daysLeft << " dias ou voc? perder? esta casa.";
